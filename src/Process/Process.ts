...
export class Process {
    ...
    private async getShell(): Promise<string | false> {
        if ("win32" === os.platform()) {
            return new Promise((resolve, reject) => {
                childProcess.exec("where bash", (error, stdout) => {
                    if (null !== error) {
                        reject(error);
                    } else {
    ...
    private escapeShellArguments(args: string[]) {
        return args.map((item: string) => {
            if (/[^A-Za-z0-9_/:=-]/.test(item)) {
                ...
                item = item.replace(/\\'''/g, "\\'"); // remove non-escaped single-quote if there are enclosed between 2 escaped
            }
